...
class AuthController extends Controller{
    ...
    /**
     * @OA\Post(
     *     path="/api/auth/register-upc",
     *     summary="Registrar un nuevo usuario con rol UPC",
     *     tags={"Auth"},
     *     @OA\RequestBody(
     *         required=true,
     *         @OA\JsonContent(
     ...
     *     @OA\Response(
     *         response=201,
     *         description="Usuario UPC registrado exitosamente"
     *     ),
     *     @OA\Response(
     *         response=422,
     *         description="Errores de validación"
     ...
     * )
     */
    public function registroUPC(Request $request){
        $datos = $request->all();
        ...
        try {
            ...
            $usuario->email_verified_at = null;
            $usuario->persona_id = $persona->id;
            $usuario->rol_id = $this::ROL_UPC; // Asignar rol UPC automáticamente
            $usuario->save();
        ...
        return response()->json(['message' => 'Usuario UPC registrado exitosamente'], 201);
    }
