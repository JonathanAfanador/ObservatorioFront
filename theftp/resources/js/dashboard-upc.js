...
(function () {
    ...
    async function apiGet(path) {
        ...
        if (json.data && json.data.data) {
            ...
            return { status: true, total: json.data.length, data: { data: json.data } }; // Envuelve no-paginados
        } else {
             return { status: true, total: 1, data: { data: [json] } }; // Envuelve respuestas simples
        }
    }
    ...
    function handleExportSummary() {
        ...
        try {
            ...
            doc.setFont('helvetica', 'bold');
            doc.text(totalConductores, valueOffset, y + textOffset, { align: 'right' });
            doc.setFont('helvetica', 'normal');
            y += 20;
            ...
            doc.setFont('helvetica', 'bold');
            doc.text(totalVehiculos, valueOffset, y + textOffset, { align: 'right' });
            doc.setFont('helvetica', 'normal');
            y += 20;
    ...
    function exportToPDF(data, headers, filename, title) {
        ...
        doc.autoTable({
            ...
            theme: 'striped',
            styles: { fontSize: 8 },
            headStyles: { fillColor: [34, 139, 230] } // Color azul
        });
        doc.save(filename);
    }
